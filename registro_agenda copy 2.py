...
def app():
    ...
    def carregar_dados_na_session_state(dados_json, course_id):
    ...
    def preenche_alocacao(id_curso_selecionado):
        ...
        if id_curso_selecionado not in st.session_state['registros']:
            ...
            #st.write(dados_json)
            #st.write(st.session_state['registros'])
            #st.write(st.session_state['agendas'])
    ...
    def salvar_json_(course_id, course_name):
        registros = st.session_state['registros'][course_id]
        data_para_salvar = {
            "curso": course_name,
            "periodos": {}
    ...
    def verificar_conflito():
        ...
        if conflito_interno:
            ...
            cursos_envolvidos = set(conflito_interno)
            ...
            salvar_conflitos(conflito_key, "Conflito", cursos_envolvidos, curso_selecionado, periodo_selecionado, day, time, professor)
    ...
    def salvar_conflitos(conflito_key, tipo, cursos, curso_selecionado, periodo_selecionado, day, time, professor):
...
#####################################################################################
    ...
    #st.write(id_curso_selecionado, periodo_selecionado)
